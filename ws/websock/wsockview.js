...
const WSockView = {
  ...
  computed: {
    ...
    textOut() {
      var s = "";
      this.$store.state.responses.forEach(
        (resp) => (s = JSON.stringify(resp, null, 2) + "\n\n" + s)
      );
      return s;
    },
    streamLog() {
    ...
    error() {
      return this.$store.state.error && this.$store.state.error.target
        ? this.$store.state.error.target.constructor
          ? "Unexpected error on " +
            this.$store.state.error.target.constructor.name
  ...
  methods: {
    doSend() {
      if (!this.$store.state.token) {
        this.$store.commit(
          "onError",
          "Token not set. Please setup token in settings"
        );
